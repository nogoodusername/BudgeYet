...
fun CreateHouseholdScreen(
    ...
    Column(modifier = modifier.fillMaxSize().verticalScroll(rememberScrollState()).imePadding().padding(16.dp)) {
        ...
        Text(text = "Household Name", style = famExType.labelMd, color = MaterialTheme.colorScheme.onSurfaceVariant)
        Spacer(modifier = Modifier.height(8.dp))
        OutlinedTextField(
            value = uiState.name,
            onValueChange = onNameChange,
            placeholder = { Text(text = "e.g., The Smith Family", style = famExType.bodyMd) },
            singleLine = true,
        ...
        Text(text = "Household Currency", style = famExType.labelMd, color = MaterialTheme.colorScheme.onSurfaceVariant)
        Spacer(modifier = Modifier.height(8.dp))
        CurrencyDropdown(selected = uiState.currency, onSelect = onCurrencyChange)

        Spacer(modifier = Modifier.height(16.dp))

        Text(text = "Cycle Start Day", style = famExType.labelMd, color = MaterialTheme.colorScheme.onSurfaceVariant)
        Spacer(modifier = Modifier.height(8.dp))
        ...
        Button(
            ...
            Text(text = if (uiState.isCreating) "Creating…" else "Create & Continue", style = famExType.headlineSm)
        }
    }
}
