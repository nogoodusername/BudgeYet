...
fun ForgotPinScreen(
    ...
    Column(modifier = modifier.fillMaxSize().verticalScroll(rememberScrollState()).imePadding().padding(24.dp)) {
        ...
        OutlinedTextField(
            ...
            placeholder = { Text(text = "e.g. name@family.com", style = famExType.bodyMd) },
            singleLine = true,
        ...
        Button(
            ...
            Text(text = if (uiState.isSubmitting) "Sending…" else "Send New PIN", style = famExType.headlineSm)
        ...
        TextButton(onClick = onBackToSignIn, modifier = Modifier.fillMaxWidth()) {
            Text(text = "Back to Sign In", style = famExType.labelMd, color = MaterialTheme.colorScheme.secondary)
        }
    }
}
