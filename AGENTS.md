...
**Expired-session auto sign-out:** The backend issues a plain access token with no refresh token (see `core/network/AuthTokenStorage.kt`), so a server-rejected token (HTTP 401) is terminal — the user must sign in again. `core/network/HttpClientFactory.kt` installs a `ResponseObserver` that fires `core/network/SessionExpiryNotifier` on any 401, and `App.kt` collects that signal to sign the user out (clearing the persisted session + token, dropping them to onboarding). This prevents the old dead end where a user whose token expired while the app was closed landed on a screen showing "Invalid or expired access token" with only a Retry button and no way to log out. The notifier has no replay buffer (a one-shot event), so it must not be used for anything where a missed signal matters.
