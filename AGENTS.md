...
**Expired-session auto sign-out:** The backend issues a plain access token with no refresh token (see `core/network/AuthTokenStorage.kt`), so a server-rejected token (HTTP 401) is terminal — the user must sign in again. `core/network/HttpClientFactory.kt` installs an `HttpResponseValidator` `handleResponseExceptionWithRequest` handler that fires `core/network/SessionExpiryNotifier` on any `ClientRequestException` with status 401, and `App.kt` collects that signal to sign the user out (clearing the persisted session + token, dropping them to onboarding). This prevents the old dead end where a user whose token expired while the app was closed landed on a screen showing "Invalid or expired access token" with only a Retry button and no way to log out. **This must be a response-exception handler, not a `ResponseObserver`:** with `expectSuccess = true`, Ktor validates the status inside the `HttpSend` plugin (`HttpCallValidator.validateResponse` throws for 401 before the response reaches the receive pipeline), so a `ResponseObserver` hooked on `HttpReceivePipeline.After` never sees a 401 on a real engine. The notifier has no replay buffer (a one-shot event), so it must not be used for anything where a missed signal matters.
...
**Keyboard/IME handling (overlap + dismissal):** The iOS wrapper (`iosApp/iosApp/ContentView.swift`) must NOT use `.ignoresSafeArea(.keyboard)` — it makes the Compose host view extend under the keyboard so the insets `imePadding()` relies on are never delivered, and every screen's fields/CTAs get covered. Keep the SwiftUI host unmodified so SwiftUI resizes it above the keyboard. Android's manifest sets `android:windowSoftInputMode="adjustResize"` on `MainActivity` for the same resize behavior. Keyboard dismissal is handled globally: `App.kt` wraps the whole tree in `core/ui/KeyboardDismiss.kt`'s `dismissKeyboardOnTap()` modifier (tap any non-field background to clear focus + hide the keyboard), and every numeric/money field uses the `dismissKeyboardOptions()` helper to give the keypad an IME "Done" action wired to hide it — iOS numeric keypads have no dismiss button otherwise. Any new screen with editable fields should apply both: leave the iOS/Android host files alone, and if the screen has a numeric field use `dismissKeyboardOptions()` (or add the Done action) rather than a bare `KeyboardOptions(keyboardType = KeyboardType.Decimal)`.
