...
"""
setup_env.py
Helper script for initial project setup of budge-yet backend.
...
def print_banner():
    print("=" * 60)
    print("      budge-yet Backend Project Setup & Database Configuration   ")
    print("=" * 60)

def prompt_db_choice(default_choice="1"):
    print("\nSelect Database Storage Option for initial setup:")
    print("  [1] Local SQLite Database (embedded, file-based: ./data/budge_yet.db)")
...
def setup_sqlite():
    ...
    db_file = input("Enter SQLite DB file path (default: ./data/budge_yet.db): ").strip()
    if not db_file:
        db_file = "./data/budge_yet.db"
...
def setup_postgres():
    ...
    dbname = input("Postgres Database Name (default: budge_yet): ").strip() or "budge_yet"
...
def prompt_cors_origins():
    ...
    domains = input(
        ...
        "(e.g. https://app.budge-yet.com) — leave blank if not deploying yet: "
    ).strip()
...
def write_env_file(config: dict):
    ...
    env_content = [
        "# Generated by scripts/setup_env.py for budge-yet Backend",
        'PROJECT_NAME="budge-yet Backend"',
        "DEBUG=True",
        'API_V1_STR="/api/v1"',
...
def setup_sqlite_non_interactive():
    ...
    return {
        "DATABASE_TYPE": "sqlite",
        "SQLITE_DB_FILE": "./data/budge_yet.db",
    }
...
def setup_postgres_non_interactive():
    ...
    return {
        ...
        "POSTGRES_PASSWORD": password,
        "POSTGRES_DB": os.getenv("POSTGRES_DB", "budge_yet"),
